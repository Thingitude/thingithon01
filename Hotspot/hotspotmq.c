/*  hotspotmq.c
 *  Takes the number of MAC addresses as argv[1]
 *  Collects sensor data - temperature, humidity, noise
 *  Constructs a message and sends it twice, once directly via Mosquitto using mqpub.c
 *  and once via The Things Network using the connected TTN UNO and ttnpub.c
 *  
 *  (c) Mark Stanley 2016
 *
...
int main (int argc, char **argv)
{
	...
	if((humAttempt==5)) {
		...
		sprintf(ttnMsg,"./ttnpub \'{\"mac\":\"%d\",\"s\":\"%d\",\"xs\":\"%d\"}\'", wifiCount, meanSound, peakSound);
	} else {
		...
		sprintf(ttnMsg,"./ttnpub \'{\"mac\":\"%d\",\"s\":\"%d\",\"xs\":\"%d\",\"h\":\"%d.%d\",\"t\":\"%d.%d\"}\'", wifiCount, meanSound, peakSound, dht11_dat[0], dht11_dat[1], dht11_dat[2], dht11_dat[3]);
	}
	write(0, 0, disp1);
	write(0, 1, disp2);
